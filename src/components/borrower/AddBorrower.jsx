...
const AddBorrower = ({ setIsBorrower }) => {
    ...
    const onSubmit = async (e) => {
        e.preventDefault();
        try {
            const result = await useAddBorrower(borrower);
            console.log(result);
            setIsBorrower(false);
    ...
    return (
        <div className="absolute top-0 left-0 flex justify-center items-center h-screen w-full bg-gray-200/50">
            <form onSubmit={onSubmit} className="flex flex-col gap-10 p-8 bg-gray-200 rounded-xl">
                <p className="text-2xl mb-3 font-medium">Add Borrower</p>
                ...
                <div className="flex gap-5">
                    <div className="flex flex-col gap-1">
                        <label htmlFor="surname" className="text-gray-500 font-medium">Amount Lended</label>
                        <input type="number" name="amountLended" onChange={handleInputChange} className="rounded-md px-4 py-2 bg-gray-100" />
                    </div>
                    <div className="flex flex-col gap-1">
